...
def main():
    ...
    for sample in rdf:
        for gene in rdf[sample]:
            ...
            outfile.write("\t".join([gene, sample, proteffect, mapping[rdf[sample][gene]["EFF"]], str(thisrec.CHROM), str(thisrec.POS), str(thisrec.POS+len(thisrec.ALT)), thisrec.REF, thisrec.ALT]))
    #print "##############################"
    outfile.close()
